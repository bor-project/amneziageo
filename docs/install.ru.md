...
`up <конфиг>` выбирает конфигурацию и подключается на ней, `select <конфиг>` только запоминает её для следующего подключения. Каждая конфигурация ходит по своему списку: `routing use <имя> <конфиг>` привязывает список, `routing use none <конфиг>` оставляет решение за собственными `AllowedIPs` конфигурации, `routing use default <конфиг>` возвращает её на список по умолчанию. Без имени конфигурации та же команда меняет сам список по умолчанию - по нему ходят все конфигурации без своего списка.
