    ...
    LogLevelController logLevel)
{
    ...
    private async Task<int> SetWebSocketAsync(string name, string toggle, string portText, string host)
    {
        ...
        await store.SetConfigTransportAsync(new ConfigTransport(name, on, host.Trim(), port, current?.Mtu ?? 1420, current?.UseIpv6 ?? false, current?.MtuMode ?? MtuMode.Auto, current?.UseRouter ?? true, current?.AllowInbound ?? false, current?.InboundNetwork ?? false, current?.ApiPort ?? 0));
