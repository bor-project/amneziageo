...
Движок AmneziaWG для Windows (`amneziawg-windows`) подключён git-сабмодулем и в основной репозиторий не входит:
...
Лаунчер одной командой поднимает бэкенд-агент и интерфейс в одном процессе. Запускать из консоли от имени администратора - он ставит службу и правила WFP и поднимает туннель; нужен собранный на шаге 2 `tunnel.dll`.
