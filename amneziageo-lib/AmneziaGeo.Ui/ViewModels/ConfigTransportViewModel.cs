...
/// <summary>
/// The transport settings of a config: the WebSocket switch with the front of the settings (the server address, the TLS port, an authorization mode - none / basic login+password / path token - with its inputs), taken only while neither the server nor the config names a front, the MTU with its mode, IPv6, the router, inbound access and routing. The mode + inputs are folded into one stored address string on save and parsed back on load. Saving sends set-websocket.
/// </summary>
internal sealed partial class ConfigTransportViewModel : ViewModelBase, IEditScope
{
    ...
    // Whether the tunnel has a front to be carried to: the one the server offers, the one the config names, or the
    // one of the settings.
    [ObservableProperty]
    [NotifyPropertyChangedFor(nameof(WebSocketOn))]
    private bool _webSocketOpen;

    // Whether the front is the one of the settings: neither the server nor the config names one.
    [ObservableProperty]
    [NotifyPropertyChangedFor(nameof(ShowWebSocketFields))]
    ...
    public ConfigTransportViewModel(IAgentConnection connection, string name, bool useWebSocket, int mtu, bool useIpv6, MtuMode mtuMode = AmneziaGeo.Decl.MtuMode.Auto, int resolvedMtu = 0, bool useRouter = true, bool allowInbound = false, bool inboundNetwork = false, string address = "", bool webSocketOpen = false, bool useRouting = true, bool routingLocked = false, bool webSocketManual = false, string endpoint = "", string webSocketHost = "", int webSocketPort = 0)
    {
        ...
        _webSocketOpen = webSocketOpen;
        _webSocketManual = webSocketManual;
        _useIpv6 = useIpv6;
    ...
    /// добавления, где конфигурации ещё нет и её сервер ещё не спрошен.
    /// </summary>
    public void SeedEndpoint(string endpoint, string front = "")
    {
        _endpoint = endpoint;
        WebSocketOpen = true;
        WebSocketManual = front.Length == 0;
    ...
    /// <summary>
    /// Whether the WebSocket address fields are shown: the switch is on and the front is the one of the settings.
    /// </summary>
    public bool ShowWebSocketFields => UseWebSocket && WebSocketManual && UiPlatform.SupportsWebSocket;
    ...
    {
        if (UseWebSocket && WebSocketManual && ConfigTransport.PortOf(WebSocketPort) < 0)
        {
    ...
    /// invalid port / MTU fails without a write and surfaces its reason, keeping the item dirty. The front of the
    /// settings travels only while it is the one the tunnel takes. Applies on reconnect.
    /// </summary>
    public async Task<bool> CommitAsync()
    {
        ...
        try
        {
            ...
            var args = new List<string>
            {
                ...
                mtuVal,
                UseIpv6 ? "on" : "off",
                MtuModes.Text(MtuModes.From(MtuMode)),
                UseRouter ? "on" : "off",
                AllowInbound ? "on" : "off",
                network ? "on" : "off",
                UseRouting ? "on" : "off",
            ...
            if (WebSocketManual)
            {
                ...
                args.Add(wsPort.ToString(CultureInfo.InvariantCulture));
                args.Add(Stored(wsPort));
            }

            var ack = await _connection.SendCommandAsync(new IpcCommand(IpcContract.OpSetWebSocket, [.. args]));
            if (ack.Ok)
            {
    ...
    private void ScheduleProbe()
    {
        ...
        ProbeFailed = false;
        if (_applying || !UseWebSocket || !WebSocketManual || !UiPlatform.SupportsWebSocket)
        {
            return;
        }
    ...
    private async Task ProbeAsync(CancellationTokenSource cts)
    {
        try
        {
            ...
            var (outcome, detail) = await EndpointProbe.CheckFrontAsync(_endpoint, Stored(port), port, cts.Token);
    ...
    // The front that stands while the fields are empty: the host and the port of the Endpoint.
    private WsEndpoint Default => WsEndpoint.Default(_endpoint, string.Empty);
