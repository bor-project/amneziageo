...
```powershell
ageo config-add proba "C:\dev\tools\amnezia\amneziageo\.local\tunnel-template.conf"
...
На стенде тестовый конфиг — `.local\tunnel-template.conf` (gitignored, содержит реальный
...
```powershell
ageo install proba "C:\dev\tools\amnezia\amneziageo\.local\tunnel-template.conf"
