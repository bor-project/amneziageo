...
internal sealed class BundleCommands(IStateStore store, GeoConfigurator geo)
{
    ...
    public async Task<IpcAck> ExportAsync(IReadOnlyList<string> args, CancellationToken ct)
    {
        ...
        foreach (var name in configNames)
        {
            ...
            var transport = await store.GetConfigTransportAsync(name, ct).ConfigureAwait(false) is { } tr
                ? new PortableBundle.TransportBlock(tr.UseWebSocket, tr.WebSocketHost, tr.WebSocketPort, tr.Mtu, tr.UseIpv6, tr.MtuMode, tr.UseRouter, tr.AllowInbound, tr.InboundNetwork)
                : null;
    ...
    private async Task ApplyConfigExtrasAsync(string name, PortableBundle.ConfigBlock block, string policy, CancellationToken ct)
    {
        ...
        {
            await store.SetConfigTransportAsync(new ConfigTransport(name, transport.UseWebSocket, transport.Host, transport.Port, transport.Mtu, transport.UseIpv6, transport.MtuMode, transport.UseRouter, transport.AllowInbound, transport.InboundNetwork), ct).ConfigureAwait(false);
        }
